export default function StudentChangePasswordModal({ show, onClose, passwordData, setPasswordData, passwordError, passwordSuccess, showPassword, setShowPassword, onSubmit }) {
  if (!show) return null;
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-md w-full p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        {passwordError && <div className="error-text mb-3 text-xs">{passwordError}</div>}
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="showPwd" checked={showPassword} onChange={() => setShowPassword(!showPassword)} className="w-3.5 h-3.5 rounded text-forest-teal focus:ring-0" />
            <label htmlFor="showPwd" className="text-xs font-normal text-gray-600">Show Password</label>
          </div>
